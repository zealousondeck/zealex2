    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (eventName === "transaction.completed") {
          try {
            const transaction = await sogoRequest<SogoTransactionLookup>(
            ...
            console.warn("[Sogo webhook] provider lookup unavailable", { eventId: payload.id, reference, eventName });
          }
        }

        const { data: result, error } = await supabaseAdmin.rpc("reconcile_sogo_transaction", {
          _event_id: payload.id,
          _provider_reference: reference,
          _provider_status: normalizedStatus,
          _verified_status: normalizeSogoStatus(verifiedStatus),
        ...
        if (error) {
          console.error("[Sogo webhook] reconciliation failed", {
            ...
            verifiedStatus: normalizeSogoStatus(verifiedStatus),
            code: error.code,
